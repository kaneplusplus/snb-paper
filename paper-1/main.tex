...
Consider the Phase II clinical trial that investigated the efficacy of 
...
then the trial would have proceed to the next stage where additional patients 
were treated. If fewer than two of the 12 Stage I patients is successful then 
the trial would have terminated. 

Our goal is to describe the distribution of the sample size of the design for 
...
specified number of success or failures is reached. As an example, suppose in 
the previously described trial, two successes were reached after enrolling 10 
...
\end{equation}
is the negative binomial probability of $s$ successful outcomes and
\begin{equation} \label{eqn:R}
T(k, p, t) = {k-1 \choose k-t} p^{k-t} (1-p)^t
\end{equation}
is the negative binomial probability of $t$ unsuccessful outcomes. 
...
To show the result in Equation (\ref{eqn:pmf}) consider the 
...
positive horizontal direction only. When the process stops, $X_k$'s and 
remains either $X_k = s$ or $X_k = k-t$. One example is given in 
...
\end{prop}
\begin{proof}
By definition, the MFG of the SNB is:
\begin{equation*}
...
Equation \ref{eqn:posterior} suggests an alternative construction for 
