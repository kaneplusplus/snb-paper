...
which the experiment ends as soon as a predefined endpoints is reached, is
...
$\mathcal{I}_p(s, t) = 1-\mathcal{I}_{1-p}(t, s)$ \citep{Uppuluri1970}.
\begin{align*}
R = \sum_{i=0}^{t-1} &{i+s-1 \choose i} p^s (1-p)^i +
\sum_{j=0}^{s-1}  {j+t-1 \choose j} p^j  (1-p)^t \\
...
\begin{equation*}
\mathbb{P} [Y \leq n\ |\ \text{trial succeeds}] 
...
\begin{align}
\mathbb{P} [Y \leq n\ |\ \text{trial succeeds}] &= \sum_{i=0}^{t-1} 
...
where $s=2$, $t=12$, and $p=0.045$.  The probability masses represented in 
...
$s$ successes (black) or $t$ failures (grey) along with Bin(0.045, 13) with
