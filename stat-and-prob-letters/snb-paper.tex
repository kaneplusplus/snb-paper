...
sample size for this trial is seven to 17.
...
\end{equation}
for $q = 1-p$ defined for $x < \min \left\{\log(1/p), \log(1/q) \right\}$.
\end{prop}
\begin{proof}
...
Let us consider the joint SNB distribution when $p$ has a beta distribution.
\begin{prop} \label{prop:posterior}
When $p$ is distributed Beta($\alpha$, $\beta$), the 
posterior density under the SNB model is
\begin{align*}
f(p | s, t, \alpha, \beta) &= 
...
\begin{corollary} \label{prop:bayesian}
...
\end{corollary}
\begin{proof}
This result can be found by integrating $p$ over the interval zero to one and
